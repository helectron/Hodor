...
header = {'User-Agent': windows_user_agent,'Referer': URL
}
...
for i in range(5):
    try:
        ...
        with requests.Session() as session:
            ...
            my_data = {'id': '2282', 'key': site_key,'captcha': captcha,'holdthedoor': 'submit'}
            # send data
            response_post = session.post(URL, headers=header, data=my_data)
            print('Post Request made with status: {}'.format(response_post.status_code))
